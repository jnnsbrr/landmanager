...
def run_landmanager(config_file):
    ...
    # with memray.Tracker("/p/projects/open/Jannes/projects/copan_lpjml/crop_calendar/python/simulations/output_file_global.bin"):
    ...
    for year in model.lpjml.get_sim_years():
        if year == model.lpjml.config.lastyear:
            profiler.stop()
            profiler.write_html("/p/projects/open/Jannes/projects/copan_lpjml/crop_calendar/python/simulations/output_test_global.html")
        model.update(year)
